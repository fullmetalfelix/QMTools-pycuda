...
CUDA_PATH = Path(__file__).resolve().parent / 'cuda'
...
class Automaton:
	...
	def Evolve(self, mol, cgrid, qref, maxiter=1000, tolerance=0.1, debug=False):
		...
		# compile
		ptx = SourceModule(kernel, include_dirs=[str(CUDA_PATH)])#, options=["--resource-usage"])
...
class AutomatonSingleShot:
	...
	def SingleShot(self, mol, cgrid, qref, debug=False, copyBack=False):
		...
		ptx = SourceModule(kernel, include_dirs=[str(CUDA_PATH)], options=opts)
...
class QMTools:
	...
	fker = open(CUDA_PATH / 'kernel_utils.cu')
	print(str(CUDA_PATH))
	source = SourceModule(fker.read(), include_dirs=[str(CUDA_PATH)], options=["--resource-usage"])
	fker.close()
	...
	def Compute_density(grid, molecule, subgrid=1, copyBack=True, debug=False):
		...
		kernel = SourceModule(kernel, include_dirs=[str(CUDA_PATH)], options=opts)
	...
	def Compute_hartree(vgrid, molecule, copyBack=True):
		...
		kernel = SourceModule(kernel, include_dirs=[str(CUDA_PATH)], options=["--resource-usage"])
		src = kernel
	...
	def Compute_VNe(grid, molecule, adsorb=0.1, diff=0.01, tolerance=1.0e-9, maxiters=None, copyBack=True):
		...
		# setup the kernel
		fker = open(CUDA_PATH / 'kernel_vne.cu')
		...
		kernel = SourceModule(kernel, include_dirs=[str(CUDA_PATH)])
	...
	def Compute_VNe_classic(grid, molecule, divisions=4, copyBack=True):
		...
		# setup the kernel
		fker = open(CUDA_PATH / 'kernel_vne_classic.cu')
		kernel = fker.read(); fker.close()
		kernel = kernel.replace('PYCUDA_NATOMS',str(molecule.natoms))
		kernel = kernel.replace('PYCUDA_NX',str(grid.shape[1]))
		kernel = kernel.replace('PYCUDA_NY',str(grid.shape[2]))
